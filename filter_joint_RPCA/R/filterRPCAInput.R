...
.validate_rpca_filter_args <- function(
  ...
  for (arg_name in names(args)) {
    ...
    if (!(is.numeric(value) && length(value) == 1L && !is.na(value))) {
      stop(
        "'", arg_name, "' must be a single numeric value or NULL.",
        call. = FALSE
      )
    }
...
.get_rpca_filter_indices <- function(
  ...
  keep_features <- rep(TRUE, nrow(mat))
  keep_samples <- rep(TRUE, ncol(mat))
  
  if (!is.null(min.feature.count)) {
    feature_totals <- rowSums(mat, na.rm = TRUE)
    keep_features <- keep_features & feature_totals > min.feature.count
  }
  
  if (!is.null(min.feature.frequency)) {
    feature_frequency <- rowSums(mat > 0, na.rm = TRUE) / ncol(mat) * 100
    keep_features <- keep_features & feature_frequency > min.feature.frequency
  }
  
  if (!is.null(min.sample.count)) {
    sample_totals <- colSums(mat, na.rm = TRUE)
    keep_samples <- keep_samples & sample_totals > min.sample.count
  }
